"""Equivalência funcional mínima da aba 7. Medição do orçamento."""
...
def _equipe_inicial():
    return (
        ...
        LinhaMaoObraMedicao(
            "ajudante-geral", "Ajudante Geral", 2, 11.13, 9, None, 110
        ),
    )
...
def _itens_iniciais():
    return (
        ItemMedicaoOrcamento("amostras", 1, "Amostras", "un", 45, 60, False),
        ItemMedicaoOrcamento(
            "batimetria-fos", 3, "Batimetria FOS", "dias", 3, None, True
        ),
        ItemMedicaoOrcamento(
            "acompanhamento-coleta-bags",
            4,
...
@dataclass(frozen=True, slots=True)
class MedicaoOrcamento:
    equipe: tuple[LinhaMaoObraMedicao, ...] = _equipe_inicial()
    custo_refeicao: float | None = 30.0
    custo_transporte: float | None = 10.0
    itens: tuple[ItemMedicaoOrcamento, ...] = _itens_iniciais()
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.equipe, tuple) or not all(
            isinstance(item, LinhaMaoObraMedicao) for item in self.equipe
        ):
            raise ValueError("Equipe da Medição inválida.")
        if not isinstance(self.itens, tuple) or not all(
            isinstance(item, ItemMedicaoOrcamento) for item in self.itens
        ):
            raise ValueError("Itens da Medição inválidos.")
        ids_equipe = {item.id for item in self.equipe}
        ids = [item.id for item in self.equipe] + [item.id for item in self.itens]
        if len(ids) != len(set(ids)):
            raise ValueError("Identificadores da Medição devem ser únicos.")
        if any(
            item.horas_referencia_id is not None
            and item.horas_referencia_id not in ids_equipe
            for item in self.equipe
        ):
            raise ValueError("Referência de horas da Medição inválida.")


@dataclass(frozen=True, slots=True)
class ResultadoLinhaMaoObraMedicao:
    id: str
...
@dataclass(frozen=True, slots=True)
class ResultadosMedicaoOrcamento:
    mao_obra: tuple[ResultadoLinhaMaoObraMedicao, ...]
    quantidade_pessoas: float
    total_refeicoes: float
    total_transportes: float
    custo_por_dia: float
    itens: tuple[ResultadoItemMedicao, ...]
    total: float
...
def calcular_medicao_orcamento(
    medicao: MedicaoOrcamento,
) -> ResultadosMedicaoOrcamento:
    """Reproduz somente as 19 fórmulas efetivamente presentes na aba."""
    zero = lambda valor: 0.0 if valor is None else valor
    horas_por_id = {}
    mao_obra = []
    for linha in medicao.equipe:
        ...
        mao_obra.append(ResultadoLinhaMaoObraMedicao(linha.id, horas, total))

    pessoas = sum(zero(item.quantidade) for item in medicao.equipe)
    refeicoes = pessoas * zero(medicao.custo_refeicao)
    transportes = pessoas * zero(medicao.custo_transporte)
    ...
    for item in medicao.itens:
        ...
        itens.append(
            ResultadoItemMedicao(
                item.id, unitario, zero(item.quantidade) * zero(unitario)
            )
        )
    total = sum(item.preco_total for item in itens)
    bdi_principal = total * zero(medicao.bdi_principal) / 100
    # F21 não existe no XML da aba. As fórmulas F22 e F23 tratam a referência
    # vazia como zero; ela não é transformada em uma nova entrada.
    bdi_secundario = 0.0 * zero(medicao.bdi_secundario) / 100
    return ResultadosMedicaoOrcamento(
        tuple(mao_obra),
        pessoas,
        refeicoes,
        transportes,
