...
class TestPainel(unittest.TestCase):
    ...
    def test_navegacao_renderiza_somente_barrilete_sem_leitura_remota(self):
        orcamento, versao = criar_orcamento_vazio("fabio").valor
        repositorio = Mock()
        estado = {
            "usuario": "fabio",
        ...
        falso = StreamlitFalso(session_state=estado, tela="Barrilete")
        with patch.object(self.painel, "st", falso), patch.object(
            self.painel.barrilete, "render"
        ) as render_barrilete, patch.object(
            self.painel.dados_obra, "render"
        ...
        render_barrilete.assert_called_once_with(
            repositorio=repositorio,
            orcamento=orcamento,
            versao=versao,
        ...
        render_producao.assert_not_called()
        repositorio.assert_not_called()

    def test_navegacao_renderiza_somente_mob_draga_sem_leitura_remota(self):
        orcamento, versao = criar_orcamento_vazio("fabio").valor
        repositorio = Mock()
        estado = {
            "usuario": "fabio",
        ...
        falso = StreamlitFalso(session_state=estado, tela="Mob. Draga")
        with patch.object(self.painel, "st", falso), patch.object(
            self.painel.mobilizacao_draga, "render"
        ) as render_mob, patch.object(
            self.painel.dados_obra, "render"
        ) as render_dados, patch.object(
            self.painel.cotacoes, "render"
        ) as render_cotacoes, patch.object(
            self.painel.producao, "render"
        ) as render_producao, patch.object(
            self.painel.barrilete, "render"
        ) as render_barrilete:
            self.painel.render(repositorio=repositorio, ao_voltar=Mock())

        render_mob.assert_called_once_with(
            repositorio=repositorio,
            orcamento=orcamento,
            versao=versao,
            snapshot_esperado="snapshot",
    ...
    def test_navegacao_renderiza_somente_mob_eq_polimero_sem_leitura_remota(self):
        orcamento, versao = criar_orcamento_vazio("fabio").valor
        repositorio = Mock()
        estado = {
            "usuario": "fabio",
        ...
        falso = StreamlitFalso(session_state=estado, tela="Mob. Eq. Polímero")
        with patch.object(self.painel, "st", falso), patch.object(
            self.painel.mobilizacao_equipamento_polimero, "render"
        ) as render_polimero, patch.object(
            self.painel.mobilizacao_draga, "render"
        ) as render_mob, patch.object(
            self.painel.dados_obra, "render"
        ) as render_dados, patch.object(
        ...
        render_polimero.assert_called_once_with(
            repositorio=repositorio,
            orcamento=orcamento,
            versao=versao,
            snapshot_esperado="snapshot",
        )
        render_mob.assert_not_called()
        render_dados.assert_not_called()
        render_cotacoes.assert_not_called()
        render_producao.assert_not_called()
        render_barrilete.assert_not_called()
    ...
    def test_navegacao_renderiza_somente_preparacao_celula_sem_leitura_remota(self):
        orcamento, versao = criar_orcamento_vazio("fabio").valor
        repositorio = Mock()
        estado = {
            "usuario": "fabio", "novo_orcamento_detalhe": (orcamento, versao),
        ...
        falso = StreamlitFalso(session_state=estado, tela="Prep. Célula")
        with patch.object(self.painel, "st", falso), patch.object(
            self.painel.preparacao_celula, "render"
        ) as render_preparacao, patch.object(self.painel.canteiro, "render") as render_canteiro:
            self.painel.render(repositorio=repositorio, ao_voltar=Mock())
        render_preparacao.assert_called_once_with(
            repositorio=repositorio, orcamento=orcamento, versao=versao,
            snapshot_esperado="snapshot",
        )
        render_canteiro.assert_not_called()
        repositorio.assert_not_called()

    def test_navegacao_renderiza_somente_fornecimento_bag_sem_leitura_remota(self):
        orcamento, versao = criar_orcamento_vazio("fabio").valor
        repositorio = Mock()
        estado = {
            "usuario": "fabio", "novo_orcamento_detalhe": (orcamento, versao),
        ...
        falso = StreamlitFalso(session_state=estado, tela="Forn. Bag")
        with patch.object(self.painel, "st", falso), patch.object(
            self.painel.fornecimento_bag, "render"
        ) as render_bag, patch.object(self.painel.preparacao_celula, "render") as render_preparacao:
            self.painel.render(repositorio=repositorio, ao_voltar=Mock())
        render_bag.assert_called_once_with(
            repositorio=repositorio, orcamento=orcamento, versao=versao,
            snapshot_esperado="snapshot",
        )
    ...
    def test_navegacao_renderiza_dragagem_apos_operacao_sem_leitura_remota(self):
        orcamento, versao = criar_orcamento_vazio("fabio").valor
        repositorio = Mock()
        estado = {
            "usuario": "fabio", "novo_orcamento_detalhe": (orcamento, versao),
        ...
        falso = StreamlitFalso(session_state=estado, tela="Dragagem")
        with patch.object(self.painel, "st", falso), patch.object(
            self.painel.dragagem, "render"
        ) as render_dragagem, patch.object(
            self.painel.operacao_sistema, "render"
        ) as render_operacao:
            self.painel.render(repositorio=repositorio, ao_voltar=Mock())
        render_dragagem.assert_called_once_with(
            repositorio=repositorio, orcamento=orcamento, versao=versao,
            snapshot_esperado="snapshot",
        )
        render_operacao.assert_not_called()
        repositorio.assert_not_called()

    def test_navegacao_renderiza_desmobilizacao_apos_dragagem_sem_leitura_remota(self):
        orcamento, versao = criar_orcamento_vazio("fabio").valor
        repositorio = Mock()
        estado = {
            "usuario": "fabio", "novo_orcamento_detalhe": (orcamento, versao),
        ...
        falso = StreamlitFalso(session_state=estado, tela="Desmob. Draga")
        with patch.object(self.painel, "st", falso), patch.object(
            self.painel.desmobilizacao_draga, "render"
        ) as render_desmobilizacao, patch.object(
            self.painel.dragagem, "render"
        ) as render_dragagem:
            self.painel.render(repositorio=repositorio, ao_voltar=Mock())
        render_desmobilizacao.assert_called_once_with(
            repositorio=repositorio, orcamento=orcamento, versao=versao,
            snapshot_esperado="snapshot",
        )
    ...
    def test_navegacao_renderiza_carga_transporte_apos_medicao_sem_leitura(self):
        orcamento, versao = criar_orcamento_vazio("fabio").valor
        repositorio = Mock()
        estado = {
            "usuario": "fabio",
        ...
        falso = StreamlitFalso(session_state=estado, tela="Carga e Transporte")
        with patch.object(self.painel, "st", falso), patch.object(
            self.painel.carga_transporte, "render"
        ) as render_carga, patch.object(
            self.painel.medicao_orcamento, "render"
        ) as render_medicao:
            self.painel.render(repositorio=repositorio, ao_voltar=Mock())
        render_carga.assert_called_once_with(
            repositorio=repositorio,
            orcamento=orcamento,
            versao=versao,
            snapshot_esperado="snapshot",
        )
        render_medicao.assert_not_called()
        repositorio.assert_not_called()
    ...
    def test_navegacao_renderiza_desmob_polimero_apos_desmob_draga_sem_leitura(self):
        ...
        falso = StreamlitFalso(session_state=estado, tela="Desmob. Eq. Polímero")
        with patch.object(self.painel, "st", falso), patch.object(
    ...
    def test_navegacao_renderiza_planilha1_apos_planilha_precos_sem_leitura(self):
        orcamento, versao = criar_orcamento_vazio("fabio").valor
        repositorio = Mock()
        estado = {
            "usuario": "fabio",
        ...
        falso = StreamlitFalso(session_state=estado, tela="Planilha1")
        with patch.object(self.painel, "st", falso), patch.object(
            self.painel.planilha1, "render"
        ) as render_planilha1, patch.object(
            self.painel.planilha_precos, "render"
        ) as render_precos:
            self.painel.render(repositorio=repositorio, ao_voltar=Mock())
        render_planilha1.assert_called_once_with(
            repositorio=repositorio,
            orcamento=orcamento,
            versao=versao,
            snapshot_esperado="snapshot",
        )
        render_precos.assert_not_called()
        repositorio.assert_not_called()
